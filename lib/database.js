    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `create_user`(\n" +
            ...
            "BEGIN\n" +
                ...
                "IF numCount = 0 THEN\n" +
                    ...
                    "VALUES (checking_account_type_id, new_user_id, 5000.00);\n" +

                    "INSERT INTO account (account_type_id, user_id, current_balance)\n" +
                    "VALUES (savings_account_type_id, new_user_id, 10000.00);\n" +
                "ELSE\n" +
                    "SET result = 1;\n" +
                "END IF;\n" +
                "COMMIT;\n" +
...
function addDummyDataToDatabase() {
    ...
    con.query(sql, ['John', 'Smith', 'example@example.com', '518210a7b7adc34a3aac2d440bb3a2796a07e3bcc918783559528b44ca5ab26a', 'dc1998bcdb6320d'], function(err, rows) {
      ...
      con.query('SELECT @result AS result', (error, data) => {
        if (error) {
          console.log(error.message);
          throw error;
        }
        ...
        if(result == 0) {
          console.log(`Successfully added John Smith user to user table`);
        } else {
          console.log(`Something went wrong adding John Smith user to user table`);
        }
      });
    });  
    ...
    con.query(sql, ['Debbie', 'Sue', 'example2@example.com', '518210a7b7adc34a3aac2d440bb3a2796a07e3bcc916156154894651561a', 'dc196sd5f4sdf6sdfd'], function(err, rows) {
      ...
      con.query('SELECT @result AS result', (error, data) => {
        ...
        let result = data[0].result;
        if(result == 0) {
          console.log(`Successfully added Debbie Sue user to user table`);
        } else {
          console.log(`Something went wrong adding Debbie Sue user to user table`);
        }
      });
    });
    ...
    con.query(sql, ['William', 'Turner', 'example3@example.com', '518210a7b7adc34a3aac2d440bbsdfjl9asdfoi20j51561a', 'ssdf5464d651ssdfsfd'], function(err, rows) {
      ...
      con.query('SELECT @result AS result', (error, data) => {
        ...
        let result = data[0].result;
        if(result == 0) {
          console.log(`Successfully added William Turner user to user table`);
        } else {
          console.log(`Something went wrong adding William Turner user to user table`);
        }
      });
    });

    // Transfer some money
    sql = `CALL transfer_money(?, ?, ?, ?, @result_message);`;

    con.query(sql, [1, 3, 5000.00, 'Draining funds from checking account for John'], function(err, rows) {
        if (err) {
        console.log(err.message);
        throw err;
        }
      ...
      // Grab the output variable
      con.query('SELECT @result_message AS result', (error, data) => {
        if (error) {
          console.log(error.message);
          throw error;
        }
        ...
        if(result == 'Transaction successful') {
          console.log(`Successfully drained checking account for John`);
        } else {
          console.log(`Something went wrong moving money from checking account`);
        }
      });
    });
    ...
    con.query(sql, [2, 3, 10000.00, 'Draining funds from savings account for John'], function(err, rows) {
        if (err) {
        console.log(err.message);
        throw err;
        }
      ...
      con.query('SELECT @result_message AS result', (error, data) => {
        ...
        let result = data[0].result;
        if(result == 'Transaction successful') {
          console.log(`Successfully drained savings account for John`);
        } else {
          console.log(`Something went wrong moving money from savings account`);
        }
      });
    });

    // Bump william up to employee
    /*
      To run this, change the user_login_id to whatever William's id will be, randomly generated everytime
    */
    sql = `CALL change_user_type(?, ?, @result_message);`;

    // con.query(sql, [5508, 'employee'], function(err, rows) {
    //     if (err) {
    //     console.log(err.message);
    //     throw err;
    //     }
    ...
    //     let result = data[0].result;
    //     if(result == 'Success updating role') {
    //       console.log(`Successfully elevated William to employee`);
    //     } else {
    //       console.log("ERROR: ", result);
    //       console.log(`Something went wrong elevating william to employee`);
    //     }
    //   });
    // });

    // Bump John up to admin
    sql = `CALL change_user_type(?, ?, @result_message);`;
    /*
      To run this, change the user_login_id to whatever John's id will be, randomly generated everytime
    */
    // con.query(sql, [6429, 'admin'], function(err, rows) {
    //     if (err) {
    //     console.log(err.message);
    //     throw err;
    //     }
    ...
    //     let result = data[0].result;
    //     if(result == 'Success updating role') {
    //       console.log(`Successfully elevated John to admin`);
    //     } else {
    //       console.log("ERROR: ", result);
    //       console.log(`Something went wrong elevating John to admin`);
    //     }
    //   });
    // });
}
