...
function createTables() {
    ...
    sql = "CREATE TABLE IF NOT EXISTS users (\n" +
                ...
                "user_login_id int NOT NULL UNIQUE,\n" +
...
function createStoredProcedures() {
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `get_salt_and_user_type`(\n" +
              "IN user_login_id VARCHAR(255)\n" +
          ")\n" +
          "BEGIN\n" +
              "SELECT u.salt, ut.user_type\n" +
              "FROM users u\n" +
              "INNER JOIN user_type ut ON ut.user_type_id = u.user_type_id\n" +
              "WHERE u.user_login_id = user_login_id\n" +
              "LIMIT 1;\n" +
          "END;";
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `update_password`(\n" +
                ...
                "BEGIN\n" +
                    ...
                    "FROM users\n" +
                    "WHERE users.user_login_id = existing_login_user_id;\n" +
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `transfer_money`(\n" +
                    "IN source_user_login_id INT,\n" +
                    "IN source_account_type VARCHAR(255),\n" +
                    "IN destination_user_login_id INT,\n" +
                    "IN destination_account_type VARCHAR(255),\n" +
                ...
                "BEGIN\n" +
                    ...
                    "DECLARE source_user_id INT;\n" +
                    "DECLARE destination_user_id INT;\n" +
                    ...
                    "START TRANSACTION;\n" +
                    "CALL get_user_id_from_login_id(source_user_login_id, source_user_id);\n" +

                    "CALL get_account_id(source_user_id, source_account_type, source_account_id);\n" +

                    "IF transaction_type = 'transfer' THEN\n" +
                      "CALL get_user_id_from_login_id(destination_user_login_id, destination_user_id);\n" +
                      "CALL get_account_id(destination_user_id, destination_account_type, destination_account_id);\n" +
                    "END IF;\n" +
                    ...
                    "IF transaction_type = 'deposit' THEN\n" +
                      ...
                      "VALUES (amount, NULL, source_account_id, memo,\n" +
                      "(SELECT NOW())\n" +
                      ");\n" +
                    ...
                    "ELSEIF source_balance >= amount THEN\n" + 
                      "IF transaction_type = 'transfer' THEN\n" +
                        "UPDATE account\n" +
                        "SET current_balance = current_balance - amount\n" +
                        "WHERE account_id = source_account_id;\n" +
                      ...
                      "ELSE\n" + 
                        ...
                        "INSERT INTO transaction (transaction_amount, sending_account_id, receiving_account_id, memo, timestamp)\n" +
                        "VALUES (amount, source_account_id, NULL, memo,\n" +
                        "(SELECT NOW())\n" +
                        ");\n" +
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `change_user_type`(\n" +
            ...
            "BEGIN\n" +
                ...
                "IF current_user_id IS NOT NULL THEN\n" +
                  ...
                  "END IF;\n" +
                "ELSE\n" +
                  "SET result_message = 'User does not exist';\n" +
                "END IF;\n" +
            "END;";
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `get_user_details`(\n" +
                "IN in_user_login_id INT\n" +
            ")\n" +
            "BEGIN\n" +
                "SELECT first_name, last_name, user_login_id, email\n" +
                "FROM users\n" + 
                "WHERE users.user_login_id = in_user_login_id;\n" +
            "END;";
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `search_user`(\n" +
              "IN in_user_login_id INT,\n" +
              "OUT out_user_type VARCHAR(255)\n" +
            ")\n" +
            "BEGIN\n" +
                "SELECT ut.user_type INTO out_user_type\n" +
                "FROM user_type ut\n" +
                "INNER JOIN users u ON u.user_type_id = ut.user_type_id\n" + 
                "WHERE u.user_login_id = in_user_login_id\n" +
                "LIMIT 1;\n" +
            "END;";
            ...
            con.query(sql, function(err, results, fields) {
              if (err) {
                ...
                console.log("database.js: procedure search_user created if it didn't exist");
              }
            });
...
function addDummyDataToDatabase() {
    ...
    sql = `CALL change_user_type(?, ?, @result_message); select @result_message;`;

    con.query(sql, [1536, 'admin'], function(err, rows) {
        if (err) {
        console.log(err.message);
        throw err;
        }
        ...
        if(result == "Success updating role") {
          console.log("database.js: Successfully updated role");
        } else {
          console.log("database.js: Something went wrong elevating roles");
        }
    });
}
