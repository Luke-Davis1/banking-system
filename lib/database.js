...
function createStoredProcedures() {
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `create_user`(\n" +
            ...
            "BEGIN\n" +
                ...
                "DECLARE new_user_login_number INT;\n" +
                "SET result = 0;\n" +
                "SET new_user_login_number = FLOOR(1000 + RAND() * 9000);\n" +
                "SELECT COUNT(*) INT numCount FROM user WHERE user.user_login_id = new_user_login_id;\n" +
                "START TRANSACTION;\n" + 
                "IF numCount = 0 THEN\n" +
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `verify_user_password`(\n" +
                ...
                ")\n" +
                "BEGIN\n" +
                    "DECLARE user_with_attempted_password INT;\n" +
                    "SELECT user_id INTO user_with_attempted_password\n" +
                    "FROM user\n" +
                    "WHERE user_login_id = existing_user_login_id\n" +
                    "AND hashed_password = attempted_hashed_password\n" +
                    "AND salt = attempted_salt;\n" +

                    "IF user_with_attempted_password IS NOT NULL THEN\n" +
                        "SET valid_password = 1;\n" +
                    "ELSE\n" +
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `update_password`(\n" +
                ...
                "BEGIN\n" +
                    ...
                    "WHEN SQLEXCEPTION THEN\n" +
                        "ROLLBACK\n" +
                        "SET result_message = 'Failed to update password';\n" +
                "END;";
